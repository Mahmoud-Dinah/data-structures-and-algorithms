...
const totalSum = (input) => {
  let total = input.reduce((acc1,value )=> {
    return acc1+=value.reduce((acc,cur)=> acc+=cur); },0);
  return total;
};
...
let findMaleAndFemale = (data) => {
  return data.filter(value => value.gender === 'male' || value.gender=== 'female').map(value1 => value1.name).join(' and ');
};
